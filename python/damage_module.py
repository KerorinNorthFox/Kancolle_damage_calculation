...
def vitality_func(armor):
    armor = int(armor)
    vitality = 0.7 * armor + 0.6 * random.randint(0, armor)
    return vitality

#ダメージ
def damage_func(final_attackpower, vitality, correction_amount_of_ammo):
    return math.floor((final_attackpower - vitality) * correction_amount_of_ammo)
